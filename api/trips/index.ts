...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (category && category !== 'all' && typeof category === 'string') {
    result = result.filter((trip) => trip.category.toLowerCase() === category.toLowerCase());
  }
  
  if (season && season !== 'all' && typeof season === 'string') {
    result = result.filter((trip) => trip.season.toLowerCase() === season.toLowerCase());
  }
  
  if (destination && destination !== 'all' && typeof destination === 'string') {
    result = result.filter((trip) => trip.destination.toLowerCase() === destination.toLowerCase());
  }
