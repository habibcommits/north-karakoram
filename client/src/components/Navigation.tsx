...
function FlyoutMenuItem({ label, items, onClose }: { label: string; items: NavItem[]; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className={cn("flex items-center justify-between px-4 py-2.5 text-base text-white font-bold cursor-pointer rounded-md transition-all duration-200 mx-1", isHovered ? "opacity-100" : "opacity-80")}>
        <span>{label}</span>
        ...
        <div className="absolute left-full top-0 ml-1 min-w-[250px] bg-[#005a50] rounded-lg shadow-xl border border-white/20 py-2 z-50">
          {items.map((item) => (
            <Link
              ...
              href={item.href}
              onClick={onClose}
              className="block px-4 py-2.5 text-base text-white font-bold rounded-md transition-all duration-200 mx-1 mb-1 last:mb-0 opacity-80 hover:opacity-100"
            >
              {item.label}
            </Link>
          ))}
    ...
    <div className="relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <button
        className={cn(
          ...
          (isActive || isOpen) ? "opacity-100" : "opacity-80 hover:opacity-100"
        )}
      >
        {trigger}
  ...
  return (
    <div className="border-b border-white/10">
      <button
        ...
        className={cn("w-[calc(100%-1rem)] mx-2 my-1 flex items-center justify-between px-4 py-3 text-white text-base font-bold rounded-md transition-all duration-200", isOpen ? "opacity-100" : "opacity-80 hover:opacity-100")}
      >
        <span>{label}</span>
        ...
        <div className="bg-black/10">
          {isCategory ? (
            Object.entries(items as NavCategory).map(([cat, subItems]) => (
              <div key={cat}>
                <button
                  ...
                  className="w-full flex items-center justify-between px-8 py-2 text-white text-base font-bold transition-all duration-200 opacity-80 hover:opacity-100"
                >
                  <span>{cat}</span>
                  <ChevronDown className={cn("w-4 h-4", openSub === cat && "rotate-180")} />
                </button>
                {openSub === cat && subItems.map((item, index) => (
                  <Link key={item.label} href={item.href} onClick={onClose} className="block px-12 py-2 text-white text-base font-bold transition-all duration-200 opacity-80 hover:opacity-100">
                    {item.label}
                  </Link>
                ))}
              </div>
            ...
            (items as NavItem[]).map((item, index) => (
              <Link key={item.label} href={item.href} onClick={onClose} className="block px-8 py-2 text-white text-base font-bold transition-all duration-200 opacity-80 hover:opacity-100">
                {item.label}
              </Link>
            ))
          )}
          ...
          <div className="container mx-auto px-4">
            <div className="flex h-[65px] items-center justify-between gap-4">
              <nav className="hidden lg:flex items-center gap-1">
                <Link href="/" className={cn("px-3 py-2 text-base font-bold text-white transition-all duration-200", location === "/" ? "opacity-100" : "opacity-80 hover:opacity-100")}>HOME</Link>
                ...
                <Link href="/travel-info" className={cn("px-3 py-2 text-base font-bold text-white transition-all duration-200", location === "/travel-info" ? "opacity-100" : "opacity-80 hover:opacity-100")}>TRAVEL INFO</Link>
                <DropdownMenu trigger="ABOUT US" items={aboutItems} isActive={location === "/team"} />
                <Link href="/contact" className={cn("px-3 py-2 text-base font-bold text-white transition-all duration-200", location === "/contact" ? "opacity-100" : "opacity-80 hover:opacity-100")}>CONTACT</Link>
              </nav>
