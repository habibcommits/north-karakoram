...
const peakCategories = [
  ...
  { id: "8000m", label: "8000m+", range: [8000, 9000], badge: "Elite" },
  { id: "7000m", label: "7000m+", range: [7000, 8000], badge: "Advanced" },
  { id: "6000m", label: "6000m+", range: [6000, 7000], badge: "Challenging" },
];
...
const getCategoryColor = (altitude: string): string => {
  const alt = getAltitudeNumber(altitude);
  if (alt >= 8000) return "bg-gradient-to-r from-amber-500 to-orange-500";
  if (alt >= 7000) return "bg-gradient-to-r from-purple-500 to-indigo-500";
  if (alt >= 6000) return "bg-gradient-to-r from-emerald-500 to-teal-500";
  return "bg-gray-500";
};
      ...
      <div className="group relative bg-white rounded-2xl lg:rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 cursor-pointer h-full flex flex-col">
        ...
        <div className="p-4 sm:p-5 lg:p-6 flex flex-col flex-grow">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-100">
            ...
            <button className="w-full justify-center flex items-center gap-1.5 sm:gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl bg-[#006F61] hover:bg-[#005a4d] text-white text-xs sm:text-sm font-semibold transition-all duration-300 hover:shadow-lg hover:shadow-[#006F61]/30 group/btn">
...
export default function Expeditions() {
  ...
  const filteredExpeditions = expeditions.filter((exp) => {
    ...
    if (selectedPeakCategory === "8000m") matchesPeak = altNumber >= 8000;
    if (selectedPeakCategory === "7000m") matchesPeak = altNumber >= 7000 && altNumber < 8000;
    if (selectedPeakCategory === "6000m") matchesPeak = altNumber >= 6000 && altNumber < 7000;
